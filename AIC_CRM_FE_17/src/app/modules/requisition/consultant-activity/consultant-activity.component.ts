...
export class ConsultantActivityComponent {
  ...
  getSingleClient(): void {
    this.apiService
      .getDataById('Client/SingleClientGet', { id: this.activityData.clientId })
      .subscribe({
        next: (response) => {
  ...
  getClientManager(): void {
    this.apiService
      .getDataById('Client/ClientManagerGet', {
        id: this.activityData.managerId,
      })
      .subscribe({
        next: (response) => {
  ...
  searchConsultantsForActivity() {
    this.apiService
      .getDataByName('Consultant/SearchConsultantsForActivity', {
        ...
        next: (res) => {
          if (res.succeeded) {
            ...
            const consultantIds = new Set(
              this.activityConsultantSideList.map((item) => item.consultantId)
            );
            this.activitySearchedData = this.activitySearchedData.map(
              (item) => {
  ...
  saveConsultantActivity(data: ConsultantActivityModal): void {
    let saveActivityData: ConsultantActivityModal =
      new ConsultantActivityModal();
    ...
    this.apiService
      .saveData('Consultant/ConsultantActivityAddUpdate', saveActivityData)
      .subscribe({
        next: (res) => {
          if (res.succeeded) {
            if (res.data.id > 0) {
  ...
  getInterviewProcessConsultantsList(requisitionId: number): void {
    this.apiService
      .getData('Consultant/InterviewProcessConsultantsList', { requisitionId })
      .subscribe({
        next: (response) => {

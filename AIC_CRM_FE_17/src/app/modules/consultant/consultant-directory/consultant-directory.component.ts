...
export class ConsultantDirectoryComponent {
  ...
  searchFields: string[] = [];
  inputSearch?: string;
  @ViewChildren('filterInput') filterInputs!: QueryList<
      ...
      next: (response) => {
        ...
        if (response.data.length > 0) {
          ...
          this.consultantsList.forEach((i) => {
            ...
            const address1 = i.address1
              ? this.stripHtml(i.address1).trim()
              : '';
            const country = i.country || '';
            const state = i.state || '';
            const city = i.city || '';
            const zip = i.zipCode || '';
            i.fullName = `${i.firstName || ''} ${i.middleName || ''} ${
              i.lastName || ''
            }`.trim();
          ...
          this.consultantsList = (response?.data || []).sort(
            (a: any, b: any) => {
          ...
          this.searchFields = response.data.length
  ...
  consultantDashboard(id: number) {
    this.router.navigate(['/consultant-dashboard', id], {
      state: {
        resumeSearchHighLight: this.inputSearch,
  ...
  deleteConsultant(id: number) {
    if (confirm('Are you sure you want to delete this consultant?')) {
      this.apiService
        .deleteData('Consultant/ConsultantDelete', { id })
        .subscribe({
          next: () => {
  ...
  onTableFilter(event: any) {
    this.fildteredConsultantsIds = (event.filteredValue || []).map(
      (row: any) => row.id
    );

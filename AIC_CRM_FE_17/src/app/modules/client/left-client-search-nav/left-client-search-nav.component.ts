...
export class LeftClientSearchNavComponent implements OnInit, OnDestroy {
  constructor(
  ...
  ngOnInit(): void {
    const savedPage = localStorage.getItem('clientListPage');
  ...
  onAlphabetFilter(letter: string): void {
    ...
    if (text) {
      filtered = filtered.filter((c) => c.name.toLowerCase().includes(text));
    }
    ...
    if (letter === 'All') {
      ...
      filtered = filtered.filter((c) =>
        c.name.toLowerCase().startsWith(lowerLetter)
      );
    }
  ...
  goToDashboard(id: number): void {
    ...
    if (client) {
      localStorage.setItem('clientListPage', this.currentPage.toString());
  ...
  ngOnDestroy(): void {
    localStorage.removeItem('clientListPage');
  }
}
